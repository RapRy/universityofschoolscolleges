...
const TopicWithThumbnail = ({ topic, from }) => {
  ...
  const dateString = () => {
    ...
    if (Date.parse(topic.createdAt) < Date.parse(topic.updatedAt))
      return `${
      ...
      <Grid container direction="row" spacing={2} alignItems="flex-start">
        <Grid item xs={"auto"}>
          <Avatar className={classes.avatar}>
            {additionalData.user?.username.charAt(0)}
          </Avatar>
        </Grid>
        <Grid item xs={9}>
          <Link
            to={`/forum/${additionalData.category?.name.replace(" ", "-")}/${
              topic._id
            }`}
          ...
          <Box marginBottom="5px">
            <Typography variant="body1" className={classes.stats}>
              ...
              {topic.meta.replies.length > 1
                ? `${topic.meta.replies.length} Replies`
                : `${topic.meta.replies.length} Reply`}
            </Typography>
            <Typography variant="body1" className={classes.stats}>
              <VisibilityIcon className={classes.icon} />{" "}
              {topic.meta.views.length > 1
                ? `${topic.meta.views.length} Views`
                : `${topic.meta.views.length} View`}
            </Typography>
            <Link
              to={`/forum/profile/${additionalData.user?._id}`}
              style={{ textDecoration: "none" }}
            >
              <Typography variant="body1" className={classes.stats}>
                <AccountCircleIcon className={classes.icon} />{" "}
                {additionalData.user?.username}
              </Typography>
            </Link>
          </Box>
          <Typography variant="body1" className={classes.updatesDetails}>
            {Date.parse(topic.createdAt) < Date.parse(topic.updatedAt)
              ? `updated on ${dateString()}`
              : `created on ${dateString()}`}{" "}
            in{" "}
            {
              <Link
                to={`/forum/${additionalData.category?._id}`}
                style={{ textDecoration: "none" }}
              >
                <span className={classes.span}>
                  {additionalData.category?.name}
                </span>
              </Link>
            }
          </Typography>
